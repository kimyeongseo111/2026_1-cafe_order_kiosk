...
- 주문 취소
- 결제 처리
- 할인 쿠폰 적용/제거 (정액·정률, 할인 전/후 금액 표시)
- 주문 목록 조회
- 인메모리 기반 주문 관리
...
| `쿠폰 적용 <코드>`                       | 현재 주문에 할인 쿠폰을 적용합니다.        |
| `쿠폰 제거`                              | 현재 주문의 쿠폰을 제거합니다.             |
| `쿠폰 목록`                              | 사용 가능한 쿠폰 목록을 조회합니다.        |
| `도움말`                                 | 사용 가능한 명령어를 출력합니다.           |
| `종료`                                   | 프로그램을 종료합니다.                     |

가격과 금액은 원 단위 정수로 처리합니다.

기본 제공 쿠폰은 `WELCOME`(10% 할인), `SAVE1000`(1,000원 할인)입니다. 할인액은 주문 금액을 넘지 않도록 계산합니다.
